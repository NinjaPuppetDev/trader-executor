...
interface LogEntry {
    ...
    prompt: string;
    response: string;
    status: 'processing' | 'completed' | 'failed' | 'readonly';
    decisionTx?: string;
    blockNumber: number;
    createdAt: string;
    ...
    const fetchLogs = async () => {
        try {
            ...
            const processedLogs = data
                .filter((log: any) => log?.id && log.createdAt)
                                            ...
                                            {logs.map((log) => (
                                                <tr key={log.id} className="hover:bg-gray-750/50 transition-colors">
                                                    ...
                                                    <td className="px-4 py-3 text-sm">
                                                        <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${log.status === 'completed' ? 'bg-green-900/30 text-green-400 border border-green-800' :
                                                                log.status === 'failed' ? 'bg-red-900/30 text-red-400 border border-red-800' :
                                                                    log.status === 'processing' ? 'bg-yellow-900/30 text-yellow-400 border border-yellow-800' :
                                                    ...
                                                    <td className="px-4 py-3 text-sm">
                                                        {log.decisionTx ? (
                                                            <a
                                                                href={`${process.env.NEXT_PUBLIC_BLOCK_EXPLORER_URL}/tx/${log.decisionTx}`}
                                                                target="_blank"
                                                            ...
                                                            >
                                                                <span className="truncate max-w-[80px]">
                                                                    {log.decisionTx.slice(0, 6)}...{log.decisionTx.slice(-4)}
                                                                </span>
